    ...
    <div className="flex flex-col h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <header className="bg-emerald-900 text-white p-4 flex justify-between items-center shadow-lg border-b-4 border-emerald-500">
        ...
        <div className="text-right hidden md:block border-l border-emerald-700 pl-4">
          ...
          <p className="text-[10px] text-emerald-400 font-bold uppercase">Tahun Ajaran 2025/2026</p>
        </div>
      </header>

      <div className="flex flex-1 overflow-hidden">
        {/* SIDEBAR DENGAN MENU TAMBAHAN */}
        <aside className="w-64 bg-white border-r border-slate-200 p-6 flex flex-col shadow-xl z-20 overflow-y-auto">
          <nav className="space-y-1 flex-1">
            ...
            <div className="my-4 border-t border-slate-100 pt-4">
              ...
              <button onClick={() => setMenuAktif('siswa')} className={`w-full text-left p-3 rounded-xl flex items-center gap-3 transition-all font-semibold ${menuAktif === 'siswa' ? 'bg-emerald-700 text-white' : 'text-slate-500 hover:bg-emerald-50'}`}>
                👥 <span>Data Siswa</span>
              </button>
              <button onClick={() => setMenuAktif('ortu')} className={`w-full text-left p-3 rounded-xl flex items-center gap-3 transition-all font-semibold ${menuAktif === 'ortu' ? 'bg-emerald-700 text-white' : 'text-slate-500 hover:bg-emerald-50'}`}>
                👨‍👩‍👧 <span>Data Orang Tua</span>
              </button>
              <button onClick={() => setMenuAktif('alamat')} className={`w-full text-left p-3 rounded-xl flex items-center gap-3 transition-all font-semibold ${menuAktif === 'alamat' ? 'bg-emerald-700 text-white' : 'text-slate-500 hover:bg-emerald-50'}`}>
                📍 <span>Data Alamat</span>
              </button>
              <button onClick={() => setMenuAktif('aktivitas')} className={`w-full text-left p-3 rounded-xl flex items-center gap-3 transition-all font-semibold ${menuAktif === 'aktivitas' ? 'bg-emerald-700 text-white' : 'text-slate-500 hover:bg-emerald-50'}`}>
          ...
          <div className="mt-auto pt-6 border-t border-slate-100 text-center">
            <p className="text-[11px] text-emerald-800 font-bold">min7ponorogo@gmail.com</p>
          </div>
        </aside>
        ...
        <main className="flex-1 overflow-y-auto p-8 bg-slate-50/50">
            ...
            <div className="space-y-6">
              <div className="bg-gradient-to-r from-emerald-900 via-emerald-800 to-emerald-700 p-10 rounded-3xl text-white shadow-2xl relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <p className="opacity-80 text-sm max-w-md">Integrasi Data EMIS: Siswa, Orang Tua, Alamat, dan Aktivitas Belajar.</p>
                </div>
                <div className="absolute right-10 top-1/2 -translate-y-1/2 text-9xl opacity-10">🕌</div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {[
                  { label: "Total Siswa", val: siswa.length, color: "text-emerald-700" },
                  { label: "Data Alamat", val: alamat.length, color: "text-blue-600" },
                  { label: "Data Orang Tua", val: orangTua.length, color: "text-orange-600" },
                  { label: "Catatan Aktivitas", val: aktivitas.length, color: "text-purple-600" }
                ].map((stat, i) => (
          ...
          {/* Bagian Tabel (Siswa, Ortu, Alamat, Aktivitas) */}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-lg overflow-hidden">
              <div className="p-6 border-b flex flex-col md:flex-row justify-between items-center gap-4">
                <h3 className="font-black text-slate-800 text-lg uppercase">
                  {menuAktif === 'siswa' && "Data Peserta Didik"}
                  {menuAktif === 'ortu' && "Data Orang Tua / Wali"}
                  {menuAktif === 'alamat' && "Data Domisili Siswa"}
                  {menuAktif === 'aktivitas' && "Log Aktivitas Belajar"}
                </h3>
                <input 
                  type="text" 
                  placeholder="Cari data..." 
                  className="px-5 py-2 bg-slate-100 rounded-xl text-sm outline-emerald-500 w-full md:w-64"
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-slate-50 text-slate-400 font-black uppercase text-[10px]">
                    <tr>
                      ...
                      {menuAktif === 'ortu' && <><th className="p-4">Nama Ayah</th><th className="p-4">Nama Ibu</th><th className="p-4">Pekerjaan</th></>}
                      {menuAktif === 'alamat' && <><th className="p-4">Dusun</th><th className="p-4">RT/RW</th><th className="p-4">Kecamatan</th></>}
                      {menuAktif === 'aktivitas' && <><th className="p-4">Tanggal</th><th className="p-4">Jenis Aktivitas</th><th className="p-4">Keterangan</th></>}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {filterData(
                      ...
                      <tr key={idx} className="hover:bg-emerald-50/30 transition-colors">
                        {menuAktif === 'siswa' && <><td className="p-4 font-bold">{row.NAMA}</td><td className="p-4">{row.NISN}</td><td className="p-4 font-black">KELAS {row["DITERIMA DI KELAS"]}</td></>}
                        {menuAktif === 'ortu' && <><td className="p-4 font-bold">{row.NAMA_AYAH || '-'}</td><td className="p-4">{row.NAMA_IBU || '-'}</td><td className="p-4">{row.PEKERJAAN_AYAH || '-'}</td></>}
                        {menuAktif === 'alamat' && <><td className="p-4 font-bold">{row.DUSUN || '-'}</td><td className="p-4">{row.RT}/{row.RW}</td><td className="p-4">{row.KECAMATAN || '-'}</td></>}
                        {menuAktif === 'aktivitas' && <><td className="p-4 font-bold">{row.TANGGAL || '-'}</td><td className="p-4">{row.JENIS || '-'}</td><td className="p-4">{row.KETERANGAN || '-'}</td></>}
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {/* Bagian Rombel */}
          {menuAktif === 'rombel' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in zoom-
